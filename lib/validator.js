...
var ConformaValidator = {
  ...
  length: function(options) {
    ...
    return function(field, value) {
      ...
      if(length < min) {
        err.push(new _error.ConformaValidationError(field, 'value.is.less.than.min', value, min, max));
      }

      if(max !== undefined && max < length) {
        err.push(new _error.ConformaValidationError(field, 'value.is.more.than.max', value, min, max));
      }

      return err.length ? err : undefined;
    }
  }
};
